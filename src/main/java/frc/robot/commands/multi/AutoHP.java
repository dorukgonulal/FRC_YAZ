...
public class AutoHP extends Command {
    ...
    /** Creates a new L3. */
    public AutoHP(ElevatorSubsystem elevatorSubsystem, CoralPivotSubsystem coralPivotSubsystem) {
    ...
    public void execute() {
        if(coralSwitch.get()) {
            ...
            new ParallelCommandGroup(
                new ElevatorControlCommand(elevatorSubsystem, ElevatorPositions.HP_LAST),
                new CoralPivotControl(coralPivotSubsystem, CoralPivotPosition.HP),
                new CoralIntakeOnCommand(coralSubsystem)
            );
        }
    }
}
