...
public class AlignToReefTagRelative extends Command {
  ...
  public void initialize() {
    ...
    yController.setSetpoint(isRightScore ? Constants.AprilConstants.Y_SETPOINT_REEF_ALIGNMENT
        : -Constants.AprilConstants.Y_SETPOINT_REEF_ALIGNMENT);
  ...
  public boolean isFinished() {
    ...
    // return this.dontSeeTagTimer.hasElapsed(Constants.AprilConstants.DONT_SEE_TAG_WAIT_TIME) ||
    //     stopTimer.hasElapsed(Constants.AprilConstants.POSE_VALIDATION_TIME);

    if ((Math.abs(LimelightHelpers.getTX("limelight-reef")) < 5 && LimelightHelpers.getTY("limelight-reef") > 2) || !LimelightHelpers.getTV("limelight-reef")) {
      return true;
    } else {
      return false;
    }
